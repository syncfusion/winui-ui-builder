...
**Q4. What color scheme do you want?**
- A) Light mode only
- B) Dark mode only
...
**Q5. What is your preferred MVVM framework?**
...
**Q6. What layout style best fits your UI?**
...
**Q7. What is your target screen / DPI?**
...
**Q8. Do you require accessibility support?**
...
**Q9. What is your approximate data scale?**
...
## 🔴 CRITICAL: How Stages Use `project_context`

**`project_context` object stores questionnaire answers and MUST be carried forward and applied at each stage:**

| Stage | Use `project_context` For |
|-------|---------------------------|
| **Stage 1** | Use `Q2_application_type` to guide intent interpretation (e.g., if "Dashboard" → expect grids/charts) |
| **Stage 2** | Use `Q5_mvvm_framework` to validate detected MVVM pattern matches questionnaire choice |
| **Stage 3** | Reference (layout already determined from Q6 in Stage 3) |
| **Stage 4** | ⛔ **MANDATORY** — Pre-populate ALL design decisions from project_context: Q3→color system, Q4→color mode, Q6→layout strategy, Q7→DPI approach, Q8→accessibility level. Do NOT re-ask these questions. |
| **Stage 5** | Use `Q5_mvvm_framework` to generate correct framework syntax; use `Q9_data_scale` to add virtualization if ">10k rows" |
| **Stage 6** | Use `Q9_data_scale` to recommend virtualization packages if data is large |
| **Stage 7** | Use `Q8_accessibility_support` to determine validation strictness (WCAG AA vs keyboard-only vs none) |

**⛔ RULE: Each stage MUST check if `project_context` value exists before asking user. If exists → use it; do NOT re-ask.**

**Example Stage 4 flow:**
```
Load project_context
  ├─ Q3 = "Modern"        → Set color_system = minimal flat palette
  ├─ Q4 = "Light only"    → Set color_mode = light only (no dark theme)
  ├─ Q6 = "Sidebar"       → Set layout_strategy = RelativePanel (constraint-based)
  ├─ Q7 = "4K"            → Set dpi_scaling = high-DPI aware (192 DPI+)
  └─ Q8 = "WCAG AA"       → Set accessibility_level = strict

Display to user:
  "✓ Design System Pre-Populated from Your Answers:
    • Color System: Modern (flat, minimalist palette)
    • Color Mode: Light only
    • Layout Strategy: Sidebar + Main panel
    • DPI Scaling: High-DPI (4K-ready)
    • Accessibility: WCAG 2.2 AA (strict)
   
   Ready to proceed? Or modify any of these?"
```
