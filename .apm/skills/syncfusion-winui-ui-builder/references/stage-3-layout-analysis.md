...
Reference labels in `controls.csv` (e.g., `syncfusion-winui-datagrid`) are for BM25 search **only** — not official NuGet names.

| Control Class | Skill Label | Official NuGet |
|---|---|---|
| `SfDataGrid` | `syncfusion-winui-datagrid` | `Syncfusion.SfDataGrid.WinUI` |
| `SfAvatarView` | `syncfusion-winui-avatar-view` | `Syncfusion.Core.WinUI` |
| `SfCartesianChart` | `syncfusion-winui-cartesian-chart` | `Syncfusion.Chart.WinUI` |
| `SfComboBox` | `syncfusion-winui-combobox` | `Syncfusion.Editors.WinUI` |
| `SfMaskedTextBox` | `syncfusion-winui-masked-textbox` | `Syncfusion.Editors.WinUI` |

Stage 3 generates skill labels. Stage 6 resolves them to official NuGet packages via control skill files (`.apm/skills/syncfusion-winui-<name>/SKILL.md`).
...
- ❌ Never use native WinUI controls when a valid Syncfusion control exists
...
- Script: `<project-root>/.apm/skills/syncfusion-winui-ui-builder/scripts/controls_search.cjs`
...
5. Updates `control-mapping.json` with:
   ...
   - `skill` — Skill reference (e.g., `syncfusion-winui-maskedtextbox`)
  ...
  "elements": [
    {
      ...
      "skill": "syncfusion-winui-maskedtextbox",
      "score": 18.5,
      "validation": "✓ VERIFIED"
    }
  ],
...
| Rule | Why |
|---|---|
| **Skill labels ≠ NuGet packages** | Labels (e.g., `syncfusion-winui-datagrid`) are search references only; NuGet resolution happens in Stage 6 |
...
- Each element enriched with:
  ...
  - `skill` — Skill reference for NuGet resolution (e.g., `syncfusion-winui-maskedtextbox`)
  - `score` — BM25 match confidence (0-100)
  - `validation` — "✓ VERIFIED" or "✗ FALLBACK"
- Metadata: `validation_status`, `execution_metrics`

**Consumer:** Stage 4 (Design System), Stage 5 (Code Generation), Stage 6 (Dependencies)
